...
// 要因の直積
var factors = jsPsych.randomization.factorial(fac, 4);
...
var trial = {
  ...
  raf_func: function(trial, elapsedTime, elapsedFrame){
    if (trial.up_down=="down") {
      comp_len = comp_len0_max - Math.round(elapsedTime/1000/2)*10;
    } else {
      comp_len = comp_len0_min + Math.round(elapsedTime/1000/2)*10;
    }
    console.log(comp_len);
    // 刺激の長さ変更
    jsPsych.currentTrial().stim_array[0].line_length = comp_len;

    for (var i=0; i<6; ++i) {
      trial.stim_array[i].show();
    }
  },
...
var start_experiment_procedure = {
  ...
  stimulus: '<p><b>ミュラー・リヤーの実験（極限法）</b></p>',
  choices: [" "],
  prompt: "<p>左と右の横線が出てきます。左の横線には矢羽がついています。<br/><br/>"+
    "右の横線の長さが2秒ごとに短く、または長くなります。<br/><br/>"+
    "左右の横線の長さが同じくらいに見えるようになったらスペースキーを押してください。"+
    "<br/><br/>"+
    "スペースキーを押すと始まります。"+
    "</p>"
};
...
var finish_experiment_procedure = {
  ...
  on_start: function(trial) {
    ...
    trial.stimulus = '<p>実験終了です。</p><p>ML-limit.csvというファイル名のデータファイルが自動的にダウンロードされています。Excelで開けるので確認してください。</p>'+
