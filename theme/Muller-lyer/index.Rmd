...
for (si in unique(dt$sequence_index)) {
  ...
  PSE = rbind(PSE, data.frame(comp_angle = tmp$comp_angle[1], pse = pse))
}

knitr::kable(PSE) # データの構造を確認しましょう

# 各条件ごとの平均値を計算します。
result = aggregate(pse ~ comp_angle, data = PSE, FUN = mean)
knitr::kable(result) 

# 図示
library(ggplot2)
ggplot(result, aes(comp_angle, pse)) + 
